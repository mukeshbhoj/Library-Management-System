...
{
	my ($mid,$bid) = @_;
	$query = $conn->prepare("select count(*) from issue_book ib, book b where ib.m_id = '$mid' and ib.book_id=b.book_id and b.group_id in (select group_id from book b where b.book_id='$bid') and ib.return_by = '0'");
	$query->execute();
	my @op = $query->fetchrow_array();
	return $op[0];
